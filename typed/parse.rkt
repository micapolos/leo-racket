...
(define (parsed-set $parsed $key $value)
  (struct-copy parsed $parsed
    (env (cons (cons $key $value) (parsed-env $parsed)))))

(define (parsed-ref $parsed $key)
  (define entry (assoc $key (parsed-env $parsed)))
  (cond
