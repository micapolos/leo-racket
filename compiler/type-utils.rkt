...
(define (type-dynamic? ($type : Type)) : Boolean
  (cond
    ...
    ((specification? $type) (type-dynamic? (specification-generic-type $type))) ; TODO: $type-stack
    ((recursive? $type) #t)
...
(check-equal? (type-dynamic? (specification static-type-a static-type-b)) #f)
(check-equal? (type-dynamic? (specification dynamic-type-a static-type-b)) #t)
(check-equal? (type-dynamic? (specification static-type-a dynamic-type-b)) #f)
(check-equal? (type-dynamic? (specification dynamic-type-a dynamic-type-b)) #t)
