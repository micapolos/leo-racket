...
(define (type-dynamic? ($type : Type)) : Boolean
  (cond
    ...
    ((specific? $type) (type-dynamic? (specific-body-type $type))) ; TODO: $type-stack
    ((recursive? $type) #t)
...
(check-equal? (type-dynamic? (specific dynamic-type-a static-type-b)) #f)
(check-equal? (type-dynamic? (specific static-type-a dynamic-type-b)) #t)
